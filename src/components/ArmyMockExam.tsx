...
import { ArmyCourseType } from "./ArmyCourseModal";
...
interface ArmyMockExamProps {
  course: ArmyCourseType;
  onBack: () => void;
}
...
const generateExamSections = (course: ArmyCourseType): ExamSection[] => {
  const allVerbal = getAllMCQs(armyVerbalChapters, armyVerbalExperienceMCQs);
  const allNonVerbal = getAllMCQs(armyNonVerbalChapters, armyNonVerbalExperienceMCQs);
  const allEnglish = getAllMCQs(armyEnglishChapters, armyEnglishExperienceMCQs);
  const allMath = getAllMCQs(armyMathChapters, armyMathExperienceMCQs);
  const allPakStudies = getAllMCQs(armyPakStudiesChapters, armyPakStudiesExperienceMCQs);
  const allIslamiat = getAllMCQs(armyIslamiatChapters, armyIslamiatExperienceMCQs);
  const allGK = getAllMCQs(armyGKChapters, armyGKExperienceMCQs);
  const allPhysics = getAllMCQs(armyPhysicsChapters, armyPhysicsExperienceMCQs);

  // Intelligence is common for all courses
  ...
  switch (course) {
    case "pma-long":
      // PMA Long: English, Math, Pak Studies, Islamiat, GK
      sections.push(
        { name: "English", questions: getRandomMCQs(allEnglish, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Mathematics", questions: getRandomMCQs(allMath, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Pak Studies", questions: getRandomMCQs(allPakStudies, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Islamiat", questions: getRandomMCQs(allIslamiat, 50), timeLimit: 30 * 60, required: 50 },
        { name: "GK & Current Affairs", questions: getRandomMCQs(allGK, 50), timeLimit: 30 * 60, required: 50 }
      );
      break;
    case "tcc":
      // TCC: Like PAF CAE - English, Physics, Math, GK
      sections.push(
        { name: "English", questions: getRandomMCQs(allEnglish, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Physics", questions: getRandomMCQs(allPhysics, 75), timeLimit: 40 * 60, required: 75 },
        { name: "Mathematics", questions: getRandomMCQs(allMath, 75), timeLimit: 40 * 60, required: 75 },
        { name: "GK & Current Affairs", questions: getRandomMCQs(allGK, 50), timeLimit: 30 * 60, required: 50 }
      );
      break;
    case "dssc":
      // DSSC: English, GK, Pak Studies, Islamiat
      sections.push(
        { name: "English", questions: getRandomMCQs(allEnglish, 50), timeLimit: 30 * 60, required: 50 },
        { name: "GK & Current Affairs", questions: getRandomMCQs(allGK, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Pak Studies", questions: getRandomMCQs(allPakStudies, 50), timeLimit: 30 * 60, required: 50 },
      ...
      break;
    case "ic":
      // IC: English, Math, GK, Pak Studies
      sections.push(
        { name: "English", questions: getRandomMCQs(allEnglish, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Mathematics", questions: getRandomMCQs(allMath, 50), timeLimit: 30 * 60, required: 50 },
        { name: "GK & Current Affairs", questions: getRandomMCQs(allGK, 50), timeLimit: 30 * 60, required: 50 },
      ...
      break;
    case "lcc":
      // LCC: English, GK, Pak Studies, Islamiat
      sections.push(
        { name: "English", questions: getRandomMCQs(allEnglish, 50), timeLimit: 30 * 60, required: 50 },
        { name: "GK & Current Affairs", questions: getRandomMCQs(allGK, 50), timeLimit: 30 * 60, required: 50 },
        { name: "Pak Studies", questions: getRandomMCQs(allPakStudies, 50), timeLimit: 30 * 60, required: 50 },
...
const calculateSectionResult = (
  ...
  section.questions.forEach((q, qIdx) => {
    ...
    if (answers[key] !== null && answers[key] !== undefined) {
      attempted++;
      if (answers[key] === q.correctAnswer) correct++;
    }
  });
...
const ArmyMockExam = ({ course, onBack }: ArmyMockExamProps) => {
  const [examKey, setExamKey] = useState(0);
  const sections = useMemo(() => generateExamSections(course), [course, examKey]);
      ...
      <div className="min-h-screen bg-gradient-navy">
        <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
          <div className="container py-4 flex items-center justify-between">
            <h1 className="font-display text-xl font-bold text-foreground">Army Mock Exam</h1>
        ...
        <main className="container py-12">
          <div className="text-center">
            ...
            <button onClick={onBack} className="mt-6 px-6 py-3 bg-primary text-primary-foreground rounded-xl font-semibold">Go Back</button>
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-navy">
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="container py-3">
          ...
          <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
            <div className="h-full bg-primary rounded-full transition-all duration-300" style={{ width: `${((currentQuestionIndex + 1) / currentSection.questions.length) * 100}%` }} />
          </div>
        </div>
      </header>
      ...
      <main className="container py-6 pb-32">
        <AnimatePresence mode="wait">
          <motion.div key={questionKey} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {isSelected && <CheckCircle2 className="w-5 h-5 text-primary ml-auto flex-shrink-0" />}
                    </div>
                  </motion.button>
                );
              })}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-lg border-t border-border p-4">
        <div className="container flex items-center justify-between gap-3">
          <button onClick={handlePrev} disabled={currentQuestionIndex === 0} className="flex items-center gap-1 px-4 py-3 rounded-xl border border-border text-foreground font-medium disabled:opacity-30">
            <ChevronLeft className="w-4 h-4" /> Prev
          </button>
          {currentQuestionIndex === currentSection.questions.length - 1 ? (
            <button onClick={handleSectionSubmit} className="flex-1 bg-gradient-gold text-primary-foreground py-3 rounded-xl font-bold shadow-gold text-center">
              Submit Section
            </button>
          ) : (
            <button onClick={handleNext} className="flex-1 bg-primary text-primary-foreground py-3 rounded-xl font-bold text-center flex items-center justify-center gap-1">
