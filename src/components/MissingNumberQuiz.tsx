// Quiz component for "Missing Number/Figure" type questions with SVG diagrams
import React, { useState, useCallback, useMemo } from 'react';
import { ArrowLeft, ArrowRight, CheckCircle, XCircle, RotateCcw, Lightbulb } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { MissingNumberMCQ } from '@/data/missingNumberData';
import { shuffleArray } from '@/lib/shuffleUtils';
import { motion } from 'framer-motion';

interface MissingNumberQuizProps {
  mcqs: MissingNumberMCQ[];
  title?: string;
  onBack: () => void;
}
...
const MissingNumberQuiz = ({ mcqs, title = 'Missing Number', onBack }: MissingNumberQuizProps) => {
  ...
  const shuffledOptions = useMemo(() => {
    const indices = mcqs[currentIndex].options.map((_, i) => i);
    const shuffled = shuffleArray([...indices]);
    const newCorrectIndex = shuffled.indexOf(mcqs[currentIndex].correctAnswer);
    return { indices: shuffled, correctAnswer: newCorrectIndex };
  }, [currentIndex, mcqs]);
  ...
  const handleOptionSelect = useCallback((optionIndex: number) => {
    ...
    setAnswers(prev => [...prev, { questionIndex: currentIndex, selectedOption: optionIndex, correctOption: shuffledOptions.correctAnswer, isCorrect }]);
    ...
    return (
      <div className="min-h-screen bg-background p-4">
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2 }} className="py-6">
            <div className="text-center mb-8">
              ...
              <h2 className="font-display text-2xl font-bold text-foreground mb-2">{title} Complete!</h2>
              <p className="text-muted-foreground mb-4">You scored {score} out of {totalAnswered}</p>
              <div className="flex gap-2 justify-center mb-2">
                <span className="px-3 py-1 rounded-full bg-success/20 text-success text-sm font-medium">✓ {score} Correct</span>
                <span className="px-3 py-1 rounded-full bg-destructive/20 text-destructive text-sm font-medium">✗ {mistakes.length} Wrong</span>
              </div>
            </div>
                  ...
                  {mistakes.map((mistake, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-card p-4 rounded-xl border border-border">
                        ...
                        <div className="flex justify-center mb-3 p-3 bg-muted/30 rounded-lg">
                          <Diagram />
                        </div>
                        {q.explanation && (
            ...
            <div className="flex gap-3">
              <button onClick={onBack} className="flex-1 bg-card border border-border text-foreground px-6 py-3 rounded-xl font-semibold flex items-center justify-center gap-2">
                <ArrowLeft className="w-5 h-5" /> Go Back
              </button>
              <button onClick={handleReset} className="flex-1 bg-gradient-gold text-primary-foreground px-6 py-3 rounded-xl font-semibold shadow-gold flex items-center justify-center gap-2">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-muted-foreground">
            Q{currentIndex + 1} of {mcqs.length} | Score: {score}/{answeredQuestions.size}
          </div>
        </div>

        {/* Progress bar */}
        <div className="h-2 bg-muted rounded-full mb-6 overflow-hidden">
          <div className="h-full bg-gradient-gold transition-all duration-300 ease-out" style={{ width: `${((currentIndex + 1) / mcqs.length) * 100}%` }} />
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-6">
          ...
          {/* SVG Diagram */}
          <div className="flex justify-center mb-6 p-4 bg-muted/30 rounded-lg">
            <Diagram />
          </div>

          {/* Text Options */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {shuffledOptions.indices.map((origIdx, displayIdx) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center justify-center gap-2 p-4 rounded-lg border-2 transition-all ${borderClass} ${
        ...
        <div className="flex justify-between">
          ...
          {showResult || answeredQuestions.has(currentIndex) ? (
            <Button onClick={handleNext}>
              {currentIndex === mcqs.length - 1 ? 'See Results' : 'Next'}
              <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          ) : (
            <Button disabled>Next <ArrowRight className="w-4 h-4 ml-2" /></Button>
          )}
        </div>
      </div>
    </div>
...
export default MissingNumberQuiz;
