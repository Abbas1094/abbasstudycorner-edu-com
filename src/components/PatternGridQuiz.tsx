...
const PatternGridQuiz = ({ mcqs, title = 'Pattern Grid', onBack }: PatternGridQuizProps) => {
  ...
  const shuffledOptions = useMemo(() => {
    ...
    const newCorrectIndex = shuffled.indexOf(currentQuestion.correctAnswer);
            ...
            {mistakes.length > 0 && (
              <div className="mb-8">
                <h3 className="font-display text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                  <XCircle className="w-5 h-5 text-destructive" /> Review Your Mistakes
                </h3>
                  ...
                  {mistakes.map((mistake, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-card p-4 rounded-xl border border-border">
                        ...
                        <div className="flex justify-center mb-3 p-3 bg-muted/30 rounded-lg"><Grid /></div>
                        {q.explanation && (
  ...
  // ============= QUIZ SCREEN =============
  const Grid = currentQuestion.grid;
...
export default PatternGridQuiz;
